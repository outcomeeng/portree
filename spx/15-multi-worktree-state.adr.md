...
This decision governs where portree's runtime state lives when multiple git worktrees of the same repository are active concurrently. State resides at a single canonical location addressable identically from every worktree.

## Context

**Business impact:** portree's product hypothesis targets 2+ concurrent worktrees as the default development mode. A per-worktree state file fragments the model: `portree up` from worktree A is invisible to `portree ls` from worktree B, ports allocated by one worktree collide with services started by another, and `--all` operations produce inconsistent views depending on which worktree invoked them.
...
Every command resolves the state directory as `filepath.Join(git.MainWorktreeRoot(cwd), ".portree")` regardless of which worktree invoked it, and `portree down --all --prune` runs the stop loop for every non-bare worktree before pruning orphaned state entries.

## Rationale

Three sub-rules follow from one principle: multi-worktree concurrency requires a single canonical address for state.

1. **State directory uses `MainWorktreeRoot`, not `FindRepoRoot`.** The main worktree root is the only path identical from every worktree. `FindRepoRoot` returns the caller's worktree root, which differs per worktree and fragments the state file. Config loading continues to use `FindRepoRoot` because `.portree.toml` is a per-worktree concern — a linked worktree may carry its own service overrides.
...
Every command's state I/O constructs the state directory through `git.MainWorktreeRoot`. No call site resolves `.portree/state.json` from `git.FindRepoRoot` or `os.Getwd()`. The `down` command runs its stop loop before any prune step.
...
- A command resolves the state directory from `git.FindRepoRoot` or `os.Getwd()` — both return per-worktree paths that fragment state ([review])
